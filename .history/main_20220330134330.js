...
function finalPos(initialPos, probeInstruction){
    ...
    initialPos.push(instruc[probeInstruction[0]]);
    initialPos[2] = windRose[initialPos[2]];

    var output = probeInstruction.split('').reduce((prev,curr) => {

        var cos = COS[prev[2] % 360];
        var sin = SIN[prev[2] % 360];

        console.log(prev[3], prev[2] % 360,cos,sin);

        return [
            instruc[curr] ? prev[0] + cos : prev[0], 
            instruc[curr] ? prev[1] + sin : prev[1],
            instruc[curr] == 1 ? prev[2] + prev[3] : prev[2] + instruc[curr],
            instruc[curr] == 1 ? prev[3] : instruc[curr],
        ]
        },initialPos
    );

    var cos = COS[output[2] % 360];
    var sin = SIN[output[2] % 360];
