...
function finalPos(initialPos, probeInstruction){
    ...
    var output = probeInstruction.split('').reduce((prev,curr) => {
        console.log(instruc[curr], prev[0], prev[2], Math.cos(prev[2]),prev[0], curr);

        var cos = Math.floor(Math.cos(prev[2]));
        var sin = Math.floor(Math.sin(prev[2]));

        return [
            instruc[curr] ? prev[0] + cos : prev[0], 
            instruc[curr] ? prev[1] + sin : prev[1],
            instruc[curr] == 1 ? prev[2] : instruc[curr]
        ]
        },initialPos
    );
